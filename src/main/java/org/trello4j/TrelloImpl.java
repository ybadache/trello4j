...
public class TrelloImpl implements Trello {
	...
	public org.trello4j.model.List createList(String idBoard, String name, Map<String, String> keyValueMap) {
		...
		final String url = TrelloURL.create(apiKey, TrelloURL.LIST_POST_URL).token(token).build();
		if (keyValueMap == null)
			keyValueMap = new HashMap<String, String>();
		// if (keyValueMap.containsKey("name")) keyValueMap.remove("name");
		keyValueMap.put("name", name);
	 ...
	 */
	@Override
	public org.trello4j.model.List getListByAction(String actionId, final String... filter) {
		validateObjectId(actionId);
	 ...
	 */
	@Override
	public Member getMemberCreatorByAction(String actionId, final String... filter) {
		validateObjectId(actionId);
	 ...
	 */
	@Override
	public Organization getOrganizationByAction(String actionId, final String... filter) {
		validateObjectId(actionId);
		...
		try {
			...
			if (map != null && !map.isEmpty()) {
				...
				for (String key : map.keySet()) {
					sb.append(sb.length() > 0 ? "&" : "").append(key).append("=").append(URLEncoder.encode(map.get(key), "UTF-8"));
				}
